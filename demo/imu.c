...
bool write_to_dev(UART_HandleTypeDef *huart, uint8_t reg_addr, uint8_t length, uint8_t data) {
    uint8_t buf_out[5];
    uint8_t buf_in[2];

    buf_out[0] = START_BYTE_WR;
    buf_out[1] = WRITE;
    buf_out[2] = reg_addr;
    buf_out[3] = length;
    buf_out[4] = data;

    if (HAL_UART_Transmit(huart, buf_out, sizeof(buf_out), HAL_MAX_DELAY) != HAL_OK) {
        return false;
    }
    HAL_Delay(10);

    if (HAL_UART_Receive(huart, buf_in, 2, HAL_MAX_DELAY) != HAL_OK) {
        return false;
    }
...
uint8_t* read_from_dev(uint8_t reg_addr, uint8_t length) {
    uint8_t buf_out[4];
    uint8_t *buf_in = (uint8_t *)malloc(2 + length);



    HAL_UART_Transmit(&huart4, buf_out, sizeof(buf_out), HAL_MAX_DELAY);
    HAL_Delay(10);
    HAL_UART_Receive(&huart4, buf_in, 2 + length, HAL_MAX_DELAY);
    ...
    memmove(buf_in, buf_in + 2, length);
    return buf_in;
}

void configure_imu(UART_HandleTypeDef *huart) {
    uint8_t *buf = read_from_dev(huart, CHIP_ID, 1);
    if (buf == NULL || buf[0] != BNO055_ID) {
        printf("Device ID is incorrect. Shutdown.\n");
        Error_Handler();
    }
    free(buf);

    if (!write_to_dev(huart, OPER_MODE, 1, OPER_MODE_CONFIG)) {
        printf("Unable to set IMU into config mode.\n");
        Error_Handler();
    }

    if (!write_to_dev(huart, PWR_MODE, 1, PWR_MODE_NORMAL)) {
        printf("Unable to set IMU normal power mode.\n");
        Error_Handler();
    }

    if (!write_to_dev(huart, PAGE_ID, 1, 0x00)) {
        printf("Unable to set IMU register page 0.\n");
        Error_Handler();
    }

    if (!write_to_dev(huart, SYS_TRIGGER, 1, 0x00)) {
        printf("Unable to start IMU.\n");
        Error_Handler();
    }

    if (!write_to_dev(huart, UNIT_SEL, 1, 0x83)) {
        printf("Unable to set IMU units.\n");
        Error_Handler();
    }

    if (!write_to_dev(huart, AXIS_MAP_CONFIG, 1, 0x24)) {
        printf("Unable to remap IMU axis.\n");
        Error_Handler();
    }

    if (!write_to_dev(huart, AXIS_MAP_SIGN, 1, 0x06)) {
        printf("Unable to set IMU axis signs.\n");
        Error_Handler();
    }

    if (!write_to_dev(huart, OPER_MODE, 1, OPER_MODE_NDOF)) {
        printf("Unable to set IMU operation mode into operation mode.\n");
        Error_Handler();
    }

    printf("Bosch BNO055 IMU configuration complete.\n");
}
...
*/
     ...
     sConfigOC.Pulse =(value>0)? 1200:value*-1;
     ...
     sConfigOC2.Pulse = (value>0)?value:1200;
...
int main(void)
{
  ...
  while (1) {//loopp
    ...
    uint8_t *buf = read_from_dev(&huart4, ACCEL_DATA, 45);
    if (buf != NULL) {
        for (uint8_t i = 0; i < 47 ; i++) {
        modbusDevices[slave_address].data[i+4] = buf[i];
        }
    }
